...
import { AppModule } from 'apps/gateway/src/gateway.module';
...
async function bootstrap() {
    ...
    const httpRoutes: RouteInfo[] = server.router.stack
        .map((layer: any) => {
    ...
    // 2) Thêm thủ công các socket routes
    const socketRoutes: RouteInfo[] = [
        {
            path: '/socket/chat:sendMessage',
            method: HTTPMethod.POST,
            module: 'CHATMESSAGES',
            name: 'SOCKET chat:sendMessage',
        },
        {
            path: '/socket/chat:read',
            method: HTTPMethod.POST,
            module: 'CHATMESSAGES',
    ...
    // 4) Lấy lại tất cả permission sau sync
    const allPerms = await prisma.permission.findMany({ where: { deletedAt: null } });

    // 5) Định nghĩa module cho từng role
