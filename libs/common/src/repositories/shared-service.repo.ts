...
import {
  ServiceFullInformationType,
  ServiceType,
} from '../models/shared-services.model';
import { OrderByType, SortBy, SortByType } from '../constants/others.constant';
import {
  GetServicesForProviderResType,
  GetServicesResType,
  UpdateServiceBodyType,
} from '../request-response-type/service/services.model';
...
export class ShareServicesRepository {
  ...
  async createService(
    service: ServiceType & { categories: number[] },
    userId: number,
    providerId: number,
  ) {
  ...
  async list({
    ...
    const services = data.map(({ provider, ...rest }) => ({
      ...rest,
  ...
  async updateServices(
    data: UpdateServiceBodyType,
    userId: number,
  ): Promise<ServiceFullInformationType> {
  ...
  async serviceBelongProvider(
    serviceId: number,
    providerId: number,
    roleName: Pick<RoleType, 'id' | 'name'>[],
  ) {
    ...
    if (
      ...
      roleName.every((item) => item.name !== RoleName.Admin)
    ) {
