...
export class ManageFundingGatewayController {
    ...
    @ZodSerializerDto(MessageResDTO)
    async createWidthDrawRequest(@Body() data: CreateWithdrawBodyDTO, @ActiveUser("providerId") providerId: number) {
        try {
            return await lastValueFrom(this.providerClient.send({ cmd: 'create-withdraw-request' }, { data, providerId }));
        } catch (error) {
            handleZodError(error)
        }
    }
